...
function cycleTrafficFromRecords(
  ...
  if (hasDeltaTraffic) {
    const up = inCycle.reduce(
      (sum, r) => sum + Math.max(0, Number(r.traffic_up || 0)),
      0,
    );

    const down = inCycle.reduce(
      (sum, r) => sum + Math.max(0, Number(r.traffic_down || 0)),
      0,
    );
  ...
  const hasCounters = records.some(
    (r) =>
      Number.isFinite(Number(r.net_total_up)) ||
      Number.isFinite(Number(r.net_total_down)),
  );
    ...
    const load = async () => {
      try {
        ...
        const hours = Math.min(
          ...
          Math.max(
            1,
            Math.ceil((Date.now() - start.getTime()) / 3600000) + 3,
          ),
        );
